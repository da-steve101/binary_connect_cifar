...
def trinarize( x, block_mask, nu = 1.0 ):
...
def trin_stop_grad( x, nu ):
    ...
    return x + tf.stop_gradient( trinarize( x, block_mask, nu ) - x )
...
def get_conv( x, filter_out, kernel_size, strides, nu ):
    ...
    conv_tri_weights = trin_stop_grad( conv_weights, nu )
...
def get_dense( x, outputs, nu ):
    ...
    tf.add_to_collection('losses', weight_decay)
    dense_tri_weights = trin_stop_grad( dense_weights, nu )
...
def conv_bn_relu( x, filter_out, kernel_size = 3, strides=(1,1,1,1), training = True, nu = 0.7 ):
    conv_out = get_conv( x, filter_out, kernel_size, strides, nu )
...
def inference( images, use_dropout = False, training = True ):
    ...
    with tf.variable_scope('conv1') as scope:
        cnn = conv_bn_relu( images, filt_out, training = training, nu = 0.7 )
        tf.add_to_collection( tf.GraphKeys.ACTIVATIONS, cnn )
    with tf.variable_scope('conv2') as scope:
        cnn = conv_bn_relu( cnn, filt_out, training = training, nu = 1.4 )
        cnn = max_pool( cnn )
    with tf.variable_scope('conv3') as scope:
        cnn = conv_bn_relu( cnn, filt_out * 2, training = training, nu = 1.4 )
    with tf.variable_scope('conv4') as scope:
        cnn = conv_bn_relu( cnn, filt_out * 2, training = training, nu = 1.4 )
        cnn = max_pool( cnn )
    with tf.variable_scope('conv5') as scope:
        cnn = conv_bn_relu( cnn, filt_out * 4, training = training, nu = 1.4 )
